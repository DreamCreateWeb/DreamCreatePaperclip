...
function toRows(services: ClinicService[]): Row[] {
  if (services.length === 0) return [{ name: "", description: "" }];
...
export function ServicesEditor({ initial }: { initial: ClinicService[] }) {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/owner/clinic/services", {
        method: "PATCH",
  ...
  return (
    <form className="space-y-6" onSubmit={onSubmit} noValidate>
      <ul className="space-y-4">
        {rows.map((row, i) => {
          const nameError =
            getError(`services.${i}.name`) || getError(`services.${i}`);
          const descError = getError(`services.${i}.description`);
          return (
            <li
              key={i}
            ...
            >
              ...
              <div className="mt-4 space-y-4">
                ...
                <label className="block">
                  ...
                  <textarea
                    className={`mt-2 ${fieldClass} min-h-24 resize-y`}
                    value={row.description}
                    onChange={(e) => update(i, { description: e.target.value })}
                    maxLength={280}
                    placeholder="One or two sentences patients see on the site."
                  />
                  {descError ? (
